                ...
                for track_index, track in enumerate(album.tracks):
                    with encoding_dm.Nested(
                        ...
                        command_line_parts: list[str] = [
                            ...
                            "-8",
                            "--verify",
                            '-T "ARTIST={}"'.format(track.artist.replace('"', r"\"")),
                            '-T "TITLE={}"'.format(track.title.replace('"', r"\"")),
                            '-T "ALBUM={}"'.format(track.album_title.replace('"', r"\"")),
                            f'-T "DATE={track.year}"',
                            f'-T "TRACKNUMBER={track.track_num}"',
                            '-T "GENRE={}"'.format(track.genre.replace('"', r"\"")),
                            '-T "COMMENT={}"'.format(track.comment.replace('"', r"\"")),
                            '-T "BAND={}"'.format(track.album_interpret.replace('"', r"\"")),
                            '-T "ALBUMARTIST={}"'.format(track.album_interpret.replace('"', r"\"")),
                            '-T "COMPOSER={}"'.format(track.composer.replace('"', r"\"")),
                            f'-T "TOTALTRACKS={track.num_tracks}"',
                            '"{}"'.format(str(wav_filename).replace('"', r"\"")),
                            '--output-name "{}"'.format(str(temp_filename).replace('"', r"\"")),
                        ]
